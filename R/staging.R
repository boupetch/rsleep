...
#' Trains a deep convolutional network for sleep stages classification.
#'
#' @description Trains a deep convolutional network described by Chambon & Al. in A Deep Learning Architecture for Temporal Sleep Stage Classification Using Multivariate and Multimodal Time Series (2018) on batches of generated by the `generate_batches` function.
...
#' @param batches_path Character vector of batches files.
#' @param epochs Integer. Number of epochs to train the model.
#' @return A weighted Keras model.
#' @export
train_chambon2018 <- function(batches, epochs = 10){

  val <- readRDS(batches[1])

  model <- keras::keras_model_sequential()

  model <- keras::layer_conv_2d(model,
                                filters = dim(val[[1]])[2],
                                kernel_size = c(1,32),
                                activation = 'linear',
                                input_shape = c(dim(val[[1]])[2],dim(val[[1]])[3],dim(val[[1]])[4]))

  model <- keras::layer_conv_2d(model,filters = 32, kernel_size = c(1,32), activation = 'relu')
  ...
  keras::compile(model,
                 ...
                 optimizer = keras::optimizer_adam(lr = 0.0001, decay = 1e-6),
                 metrics = "accuracy"
  )
