...
Once the model has been trained, it can be reused using the `score_stages_edf()` function. The model needs to be unserialized first. Like for the pre-trained model, `score_stages_edf()` returns the hypnodensity for a given record. In this example, the third record, not used in the training process, is used:
...
Plot it using `plot_hypnodensity()`:

```{r}
plot_hypnodensity(hypnodensity)
```

And compare it to the visual hypnogram from the database:

```{r}
plot_hypnogram(
  read_events_sleepedfx("SC4002EC-Hypnogram.edf"))
```

Of course, results are debatable. In this example, the model has been trained on too few records to produce a valuable result. However, with more records (and time), the model designed by Chambon & Al. showed accuracy close to human scorers @chambonDeepLearningArchitecture2017.
