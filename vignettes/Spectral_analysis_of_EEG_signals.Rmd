...
```{r pdg_aggregate}
avg_periodograms <- aggregate(psd ~ hz+stage,
                              periodograms_df,
                              mean)
```
...
Each sleep stage show a distinct average periodogram. If the N3 stage show higher PSD values in the lower spectrum, it show way lower PSD in the upper frequencies compared to other stages.
...
Bands can be computed using the `bands_power` of the `rsleep` package. Those bands can be normalized by the spectrum range covered by the bands. 

```{r bands_compute}
bands <- lapply(epochs,function(x){
    bands_power(bands = list(c(0.5,3.5), # Delta
                             c(3.5,7.5), # Theta
                             c(7.5,13), # Alpha
                             c(13,30)), # Beta
