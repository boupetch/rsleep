...
EEG is a major component in sleep analysis. Sleep stages are mainly defined over EEG characteristics @AASMScoringManual. Many sleep related disorders modify EEG data. Polysomnography, the gold standard exam in sleep medicine, includes EEG along many others types of records @ibanezSurveySleepAssessment2018. 

EEG data can be found in sleep records files. In European Data Format (EDF) @Kemp1992 files, EEG channels data can be found labelled under their placement names. This vignette explains how to extract EEG data from sleep recrods and perform basic spectral analysis.
...
The `.edf` file contains signals and metadata. The `.csv` files contains the scoring, exported from Noxturnal, a visual analysis software for sleep data published by Resmed.

This night has been recorded on healthy subject at the Sleep and Vigilance center of the Hôtel-Dieu, Paris, France. The R package [`edfReader`](https://cran.r-project.org/web/packages/edfReader/index.html) can read `.edf` files. Reading an `.edf` file is divided into two steps: First reading the headers of the file, then reading the signals. Signals can be selected to only read a selection of interest. The folowing spectral analysis will only be performed on a single channel of the EEG, `C3-M2`.
...
The traditional way to analyze EEG is to cut the frequencies of the spectrum into bands or ranges @niedermeyerElectroencephalographyBasicPrinciples2005. Bands limits can slightly vary in publications: 
...
periodograms <- mapply(x = epochs, y = hypnogram$event[-1], FUN = function(x,y){
  periodogram <- sm(x, sRate = s$sRate)#pwelch(x, sRate = s$sRate, show = FALSE)
...
length(pspectrum_basic(c(1:1000),verbose = TRUE,tapseq=c(1,2,3,4))$freq)

riedsid(c(1:1000),tapseq = c(1:500))
```
