...
In MDF, signals are stored as raw binary files in folders, while events and metadata are stored in JSON.

## Reading events

Scoring of the previous sample record can be recorded here.

```{r download_events}
download.file("http://cloud.frenchkpi.com/s/wreGqkitWNnWwnP/download", "15012016HD.csv")
```

These events have been exported from Noxturnal, a sleep scoring software published by Resmed. As they are stored using a particular convention, the `read_events_noxturnal` from the `rsleep` package helps us reading the file as clean dataframe.
