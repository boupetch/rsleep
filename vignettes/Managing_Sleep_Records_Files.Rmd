...
```{r plot_edf, fig.width = 7}
plot(c3m2[(c3m2sr*30):(c3m2sr*30*2)],type = "l")
```

# Events Files

Despite the EDF standard supporting events since 2003, very few software allow events exportation directly into EDF files.

Scoring of the previous sample record can be downloaded here, as a CSV file.
...
Although EDF imposed itself as a standard for data exchange, it may not be seen suitable for all usages. When finding new ways to store physiological data, one must keep in mind some simple rule to ensure its effectiveness:
