...
Get the signal and multiply it as the A7 algorithm were designed for μVolts and the current EEG record is in Volts.
...
Visualise the 3rd detected spindle with the `plot_event()` function from `rsleep`.
...
Joining the previous step in a for loop along all N2 epochs, spindles are detected over the whole sleep record:
