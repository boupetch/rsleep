...
Get the signal and multiply it as the A7 algorithms were designed for μVolts and the current record is in Volts.
...
Visualise the 3rd spindle with the `plot_event()` function from `rsleep`.
...
plot_event(
 ...
 sRate = s$sRate,
 sig_start = 0,
 event_start = result$spindles$idxStart[2]/s$sRate,
 event_end = result$spindles$idxEnd[2]/s$sRate,
 window = 3
)
...
Putting all the previous code in a for loop along N2 epochs, spindles are detected over the whole sleep record:
...
Plot the number of detected spindles by N2 epoch number using `ggplot2`:
